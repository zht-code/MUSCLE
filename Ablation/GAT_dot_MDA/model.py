...
class MDA(nn.Module):
    ...
    def forward(self, data, train_sample, test_sample, device):
        ...
        m_drug_d_ass = torch.cat((m_drug_d_ass[0][:901], m_drug_d_ass[0][1183:]), dim=0)  # 1x1778x901
        m_incRNA_d_ass = torch.cat((m_incRNA_d_ass[0][:901], m_incRNA_d_ass[0][1582:]), dim=0)  # 1x1778x901
        m_mRNA_d_ass = torch.cat((m_mRNA_d_ass[0][:901], m_mRNA_d_ass[0][3052:]), dim=0)  # 1x1778x901
        # 将四张图进行点击操作
        result1 = torch.mul(m_drug_d_ass, m_incRNA_d_ass)       #1778x901
        result2 = torch.mul(result1, m_mRNA_d_ass)
        sum_x = torch.mul(result2, data['miRNA_disease_feature'])       #1778x901
