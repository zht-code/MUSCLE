...
# 1
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'Ablation2/att_2/att_2_ROC.csv')#读取每一个消融实验下模型，保存到的FPR和TPR的结果文件
P = []
R = []
counter = 0
...
plt.plot(P, R, color='green',
         label=r'att_2_AUC (AUC = %0.4f)' % (roc_auc),linestyle='-',#修改消融实验下模型的名称
         lw=2, alpha=1)
...
# 2
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'Ablation2/att_3/att_3_ROC.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='orange',
         label=r'att_3_AUC (AUC = %0.4f)' % (roc_auc),linestyle='--',
         lw=2, alpha=1)

# 3
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'Ablation2/att_4/att_4_ROC.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='red',
         label=r'att_4_AUC (AUC = %0.4f)' % (roc_auc),linestyle='-.',
         lw=2, alpha=1)

# 4
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'Ablation2/attribuate/attribuate_ROC.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='deepskyblue',
         label=r'attribuate_AUC (AUC = %0.4f)' % (roc_auc),linestyle=':',
         lw=2, alpha=1)

# 5
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'Ablation2/network/network_ROC.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
    ...
    counter = counter + 1
roc_auc = auc(P, R)
plt.plot(P, R, color='#661D98',
         label=r'network_AUC (AUC = %0.4f)' % (roc_auc),linestyle=':',
         lw=2, alpha=1)


# 6
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'Ablation2/GAT_MSCAM_MDA/GAT_MSCAM_MDA_ROC.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='black',
         label=r'GAT_MSCAM_MDA_AUC (AUC = %0.4f)' % (roc_auc),linestyle=':',
         lw=2, alpha=1)
# MyEnlarge(0, 0.7, 0.25, 0.25, 0.5, 0, 2, P, R, 2, color='black')
...
plt.legend(bbox_to_anchor=(0.335, 0.365), fontsize='large')

plt.savefig('CompaROC5fold.pdf',dpi=600)
