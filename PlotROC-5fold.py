...
while counter0 < 5:
    ...
    RAPName ='save_model/5_fold/5fold_'+ str(counter0+1) + '_int.csv'
    RAPNameProb ='save_model/5_fold/5fold_'+ str(counter0+1) + '_pro.csv'
...
storFile(MyNew, 'save_model/5_fold/五折的表.csv')#保存最终的实验结果
print(MyNew)
...
while counter < len(mean_fpr):
    ...
    counter = counter + 1
storFile(FprAndTpr, 'save_model/5_fold/5foldROC_FprAndTprRPC.csv')#这个文件是为了和别的。消融实验的模型的结果比较，用来绘制比较的ROC图
