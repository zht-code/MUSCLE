...
while counter0 < 10:
    ...
    # MyEnlarge(-0.019, 0.79, 0.25, 0.25, 0.5, 0, 2, mean_fpr, tprs[i], 1.5, colorlist[i])
...
storFile(MyNew, 'save_model/10_fold/十折的表.csv')#保存最终的实验结果
print(MyNew)
...
while counter < len(mean_fpr):
    ...
    counter = counter + 1
# storFile(FprAndTpr, 'save_model/10_fold/10foldROC_FprAndTprRPC.csv')#这个文件是为了和别的。消融实验的模型的结果比较，用来绘制比较的ROC图


plt.plot(mean_fpr, mean_tpr, color=colorlist[10],
...
# # 保存图片
plt.savefig('ROC-10fold.pdf',dpi=600)
plt.show()
