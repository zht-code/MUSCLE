...
class MDA(nn.Module):
    ...
    def forward(self, data, train_sample, test_sample, device):
        ...
        # 构图
        m_incRNA_d_ass = self.gat_m_incRNA_d(m_incRNA_d_feature, data['m_incRNA_d_adj'], device)  # 1x2060x901

        # 将drug、mRNA、incRNA中间层去掉
        m_incRNA_d_ass = torch.cat((m_incRNA_d_ass[0][:901], m_incRNA_d_ass[0][1582:]), dim=0).unsqueeze(
            0)  # 1x1778x901
        # 将图拼在一起
        sum_x = torch.cat((m_incRNA_d_ass, data['miRNA_disease_feature'].unsqueeze(0)), 0)
